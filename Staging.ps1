...
$tempPath = Test-Path -Path "C:\Temp\alpha"
...
if($tempPath) {
    ...
    if (Test-Path -Path "C:\Temp\alpha\ChromeInstaller.exe") {
        $downloadChrome = $False
    } else { $downloadChrome = $True }
    if (Test-Path -Path "C:\Temp\alpha\gcpwstandaloneenterprise64.msi") {
        $downloadGCPW = $False
    } else { $downloadGCPW = $True }
    if (Test-Path -Path "C:\Temp\alpha\GoogleDriveSetup.exe") {
        $downloadDrive = $False
...
function InstallChrome {
    try {
        if ($TestChromeInstalled -eq $False) {
            ...
            $setupFile = "C:\Temp\alpha\ChromeInstaller.exe"
...
function InstallDrive {
    try {
        if ($TestDriveInstalled -eq $False) {
            ...
            $setupFile = "C:\Temp\alpha\GoogleDriveSetup.exe"
...
function InstallGCPW {
    try {
        if ($TestGCPWInstalled -eq $False) {
            ...
            $setupFile = "c:\Temp\alpha\gcpwstandaloneenterprise64.msi"
        ...
        if ($TestNableInstalled -eq $False) {
            if ($nAble -eq "y") {
                if (Test-Path -path "C:\Temp\alpha\nAble.exe") {
                    ...
                    Invoke-Webrequest -uri $uri -OutFile "C:\Temp\alpha\WindowsAgentSetup.exe"

                    Import-Csv 'C:\Temp\alpha\Markets.csv' | ft
                    ...
                    while ($siteID -eq "") {
                        Import-Csv C:\Temp\alpha\SiteID.csv | Where-Object { $_.Customer -eq $search } | ft
                    ...
                    Start-Process "C:\Temp\alpha\WindowsAgentSetup.exe" -ArgumentList $arguments -Wait -WindowStyle Hidden
                    Write-Host = "N-Able Installation Completed!"
                }
            }
...
function InstallSymantec {
    try {
        if ($TestSymantecInstalled -eq $False) {
            if ($sym -eq "y") {
                if (Test-Path -Path "C:\Temp\alpha\Symantec_Agent_Setup.exe") {
                    Write-Host "`nInstalling Symantec" | Green
                    Start-Process "C:\Temp\alpha\Symantec_Agent_Setup.exe" -Verb runAs -Wait
